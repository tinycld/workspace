...
**Unreleased migrations may be edited in place; released ones are frozen — append instead.** A single-tenant deployment is provisioned fresh, so while a migration is still unreleased there is no older schema to upgrade from and no backfill is required when its rules or fields change. PocketBase never re-runs an applied migration, so an in-place edit only works *because* every database starts empty — a rewritten migration silently never applies to an existing DB, leaving its rules evaluating against the old schema. Do not rely on rewriting history to fix a live database. **Once a package version is released this hardens into a rule:** a hosted org (multi-org router) upgrades its package set over time, so a version's migrations run against DBs that already applied the *previous* version's — an edited file with the same name never re-applies. A shipped version's migrations are therefore immutable; a schema change ships as a NEW migration in a NEW version. (The multi-org router's own control-plane schema already follows this — see `multi-org/internal/controlplane/schema.go`, where every change is an appended migration, never an edit to the init one.)
...
At the **workspace root**, `tinycld/scripts/write-workspace-root.ts` rewrites the root coordination files on every install: `pnpm-workspace.yaml`, the root `package-versions.json` copy, `biome.json`, and `.watchmanconfig` are fully overwritten, `package.json` keeps your human-owned fields but re-enforces its bootstrap-owned ones (`packageManager`, `workspaces`, the `postinstall` script), and `.npmrc` is only written if missing. Don't hand-edit the derived content in any of them. Vendor versions are pinned in one place, `tinycld/core/package-versions.json`; to bump one, edit that file only — never bootstrap, never a feature's own `peerDependencies` — and follow up with `git pull` + `pnpm install` (`tinycld/CONTRIBUTING.md`'s "Vendor version pins" section has the full workflow).
