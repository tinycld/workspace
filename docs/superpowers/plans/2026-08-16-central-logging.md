...
// sentry-go v0.44.1's Transport interface requires all five of these.
...
func TestSentryHandlerAttachesAttrsAsContext(t *testing.T) {
	...
	if !ok {
		t.Fatalf("expected a \"log\" context, got %v", tr.events[0].Contexts)
	}
	if logCtx["pkg"] != "cards" {
		t.Errorf("expected pkg=cards in the log context, got %v", logCtx["pkg"])
	}
	if logCtx["boardID"] != "b1" {
		t.Errorf("expected boardID=b1 in the log context, got %v", logCtx["boardID"])
	}
}
```
...
func (h *sentryHandler) Handle(ctx context.Context, r slog.Record) error {
	...
	// SetContext, not SetExtras (deprecated in sentry-go v0.44.1) and not
	// SetAttributes (its suggested replacement, which does NOT attach to error
	// events — this handler exists to capture events, so attrs would vanish).
	// Matches the precedent in coreserver/rebuild_logging.go.
...
**Install timing — the hazard that actually bites.** `app.logger` is created only inside
PocketBase's `initLogger()` (`pocketbase/core/base.go:1427`, assigned at line 1491), which
runs during **bootstrap**. `Register` runs before that, so `app.Logger()` hits its
documented fallback (`base.go:360-366`) and returns `slog.Default()`.

Installing during `Register` therefore passes the *current default logger's own handler*
into the fan-out and then installs that fan-out as the new default — the fan-out contains
itself, and the first `slog.Info` during registration loops forever. This is not
theoretical: it hung `go test ./coreserver/` at the 600s deadline, with the goroutine dump
pointing at `fanout.go:51` reached from `automation.Register`.

A second, independent reason the early placement is wrong: `BatchHandler.Handle`
ultimately writes batched records to the `_logs` **database table**, and there is no
usable DB before bootstrap.

**So `Install` must be bound to `OnBootstrap` and run after `e.Next()`**, when the real
PocketBase logger exists. `Install` still takes an already-resolved `slog.Handler` and
never calls `app.Logger()` itself. Anything logged between `Register` and bootstrap falls
through to Go's default stderr handler, which is acceptable and deliberate.
...
In `tinycld/core/server/coreserver/server.go`, inside `Register`, bind the install to
bootstrap (match the file's existing hook-binding style):

```go
	// Install on bootstrap, NOT here. app.Logger() falls back to slog.Default()
	// until initLogger() runs during bootstrap, so resolving it now would hand
	// the fan-out its own handler and the first log call would loop forever.
	// The PB handler also batches writes to the _logs table, which needs a live
	// DB. Anything logged before this point falls through to Go's default
	// stderr handler — deliberate.
...
The module is `tinycld.org/core` and its `go.mod` lives **in** `tinycld/core/server/`, so
that directory is the module root — sibling packages import as `tinycld.org/core/oauth`,
not `tinycld.org/core/server/oauth`. The logging package is therefore
`tinycld.org/core/logging`.
...
2. **A Go lint** forbidding stdlib `log` and bare `slog` in favor of `ForPackage`. Only
   ...
   Note for whoever writes these: the install-timing bug (Task 7) hung the whole
   `coreserver` suite and was invisible to the narrower `-run Sentry` check this plan
   originally specified. When a member migrates, run its FULL server suite, not a
   targeted subset — a logger misconfiguration manifests as a hang on any early log
   call, not as a focused test failure.
